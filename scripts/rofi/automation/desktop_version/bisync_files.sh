...
json_file="$HOME/scripts/rofi/automation/desktop_version/save_files_data.json"
...
sudo rsync -av --delete $HOME/desktopgit/Desktop/ $HOME/desktopgit/DesktopBackup/
...
if [ $? -eq 0 ]; then
    echo "{ \"number\": $new_number, \"date\": \"$current_date_time\", \"old_date\": \"$old_date\" }" | jq . > "$json_file"
    notify-send -a $current_time -u low -t 3000 "Files bisync to desktop" "<span foreground='#a3be8c' size='medium'>Successful</span>"
  else
    notify-send -a $current_time -u critical -t 3000 "Files bisync to desktop" "<span foreground='#bf616a' size='medium'>Faild</span>"
fi
